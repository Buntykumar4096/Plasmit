...
DOCS = ROOT / "docs"
ASSETS = DOCS / "assets" / "hms-er-diagram"
DOCX = DOCS / "Plasmit-HMS-ER-Diagram-Database-Scripts-and-Tables.docx"
MD = DOCS / "plasmit-hms-er-diagram-database-scripts-and-tables.md"
...
def draw_wrapped(
    ...
    if line:
        lines.append(line)
    heights = [draw.textbbox((0, 0), item, font=fnt)[3] for item in lines]
    ...
    for item, height in zip(lines, heights):
        ...
        width = bbox[2] - bbox[0]
        x = x1 + 14 if align == "left" else x1 + (x2 - x1 - width) // 2
        draw.text((x, y), item, font=fnt, fill=color(fill))
        y += height + 7


def box(draw: ImageDraw.ImageDraw, xy: tuple[int, int, int, int], title: str, body: str, fill: str = "FFFFFF") -> None:
    draw.rounded_rectangle(xy, radius=18, fill=color(fill), outline=color("CBD5E1"), width=3)
    x1, y1, x2, y2 = xy
    draw_wrapped(draw, title, (x1 + 8, y1 + 8, x2 - 8, y1 + 56), size=22, fill="14325C", bold=True)
    draw_wrapped(draw, body, (x1 + 10, y1 + 58, x2 - 10, y2 - 8), size=17, fill="475569")


def arrow(draw: ImageDraw.ImageDraw, start: tuple[int, int], end: tuple[int, int], fill: str = "2563EB") -> None:
    draw.line([start, end], fill=color(fill), width=4)
    sx, sy = start
    ex, ey = end
    ...
    draw.polygon(points, fill=color(fill))


def create_er_image() -> Path:
    ASSETS.mkdir(parents=True, exist_ok=True)
    img = Image.new("RGB", (1900, 1250), rgb("F8FAFC"))
    draw = ImageDraw.Draw(img)
    draw.rectangle((0, 0, 1900, 120), fill=rgb("14325C"))
    draw.text((45, 24), "Plasmit HMS High-Level ER Diagram", font=font(44, True), fill=rgb("FFFFFF"))
    draw.text((48, 82), "Core table relationships for database scripts, tables, FHIR and HL7 readiness", font=font(23), fill=rgb("D8E8FF"))

    nodes = {
        "Tenant": ((80, 190, 350, 310), "tenants", "Root SaaS customer"),
        "Hospital": ((460, 190, 730, 310), "hospitals", "FK tenant_id"),
        "Branch": ((840, 190, 1110, 310), "branches", "FK hospital_id"),
        "Users": ((1220, 190, 1490, 310), "users / roles", "RBAC + branch access"),
        "Patient": ((460, 390, 730, 520), "patients", "Hospital-level identity"),
        "Appointment": ((80, 610, 350, 740), "appointments", "Branch scheduling"),
        "Encounter": ((460, 610, 730, 740), "encounters", "Clinical anchor"),
        "Clinical": ((840, 610, 1110, 740), "clinical records", "vitals, notes, diagnosis"),
        "Lab": ((80, 850, 350, 980), "lab / radiology", "orders, results, reports"),
        "Prescription": ((460, 850, 730, 980), "prescriptions", "MedicationRequest"),
        "Pharmacy": ((840, 850, 1110, 980), "pharmacy", "stock, dispense, sales"),
        "Billing": ((1220, 850, 1490, 980), "billing", "invoice, payment, claim"),
        "Documents": ((1220, 610, 1490, 740), "documents", "reports and uploads"),
        "FHIR": ((1560, 390, 1830, 520), "FHIR mapping", "resource mapping/store"),
        "HL7": ((1560, 610, 1830, 740), "HL7 log", "ADT, ORM, ORU"),
        "Reporting": ((1560, 850, 1830, 980), "summary tables", "dashboards / AI"),
        "IPD": ((80, 1060, 350, 1190), "IPD / Bed / Ward", "admission, bed allocation"),
        "Surgery": ((460, 1060, 730, 1190), "surgery / OT", "Procedure"),
        "Audit": ((840, 1060, 1110, 1190), "audit logs", "all critical actions"),
    }

    for key, (xy, title, body) in nodes.items():
        fill_color = "EAF4FF" if key in {"Tenant", "Hospital", "Branch", "Patient", "Encounter"} else "FFFFFF"
        if key in {"FHIR", "HL7", "Reporting"}:
            fill_color = "E8F7F3"
        box(draw, xy, title, body, fill=fill_color)

    arrow(draw, (350, 250), (460, 250), "0F766E")
    arrow(draw, (730, 250), (840, 250), "0F766E")
    arrow(draw, (1110, 250), (1220, 250), "64748B")
    arrow(draw, (595, 310), (595, 390), "2563EB")
    arrow(draw, (595, 520), (595, 610), "2563EB")
    arrow(draw, (350, 675), (460, 675), "64748B")
    arrow(draw, (730, 675), (840, 675), "2563EB")
    arrow(draw, (730, 675), (1320, 850), "2563EB")
    arrow(draw, (595, 740), (215, 850), "64748B")
    arrow(draw, (595, 740), (595, 850), "64748B")
    arrow(draw, (730, 915), (840, 915), "64748B")
    arrow(draw, (1110, 915), (1220, 915), "64748B")
    arrow(draw, (1110, 675), (1220, 675), "64748B")
    arrow(draw, (1490, 455), (1560, 455), "0F766E")
    arrow(draw, (1490, 675), (1560, 675), "0F766E")
    arrow(draw, (1490, 915), (1560, 915), "0F766E")
    arrow(draw, (460, 675), (215, 1060), "64748B")
    arrow(draw, (595, 740), (595, 1060), "64748B")
    arrow(draw, (1110, 1125), (1220, 980), "64748B")

    path = ASSETS / "plasmit_hms_high_level_er_diagram.png"
...
def border(cell, border_color: str = "CBD5E1") -> None:
    tc_pr = cell._tc.get_or_add_tcPr()
    ...
    for edge in ("top", "left", "bottom", "right"):
        ...
        item.set(qn("w:sz"), "4")
        item.set(qn("w:color"), border_color)
...
def table(doc: Document, headers: list[str], rows: list[list[str]]) -> None:
    ...
    for row_index, row in enumerate(rows):
        ...
        for index, value in enumerate(row):
            ...
            run.font.name = "Aptos"
            run.font.size = Pt(7.0 if len(headers) > 5 else 8.0)
            run.font.color.rgb = RGBColor(*rgb("334155"))
    doc.add_paragraph()
...
def build_doc(er_image: Path) -> Document:
    doc = Document()
    ...
    run = p.add_run("Plasmit HMS ER Diagram")
    run.font.name = "Aptos Display"
    run.font.size = Pt(22)
    run.font.bold = True
    run.font.color.rgb = RGBColor(255, 255, 255)
    ...
    run2 = p2.add_run("Database Scripts, Tables and Relationship Blueprint")
    run2.font.name = "Aptos Display"
    run2.font.size = Pt(15)
    run2.font.bold = True
    run2.font.color.rgb = RGBColor(*rgb("D8E8FF"))
    doc.add_paragraph()
    doc.add_picture(str(er_image), width=Inches(7.2))
    doc.add_paragraph()

    heading(doc, "1. Purpose", 1)
    para(doc, "This document gives a high-level ER diagram and table relationship plan for the Plasmit HMS database. It explains database scripts, table groups, module-wise tables, and key parent-child relationships.")

    heading(doc, "2. Database and Script Order", 1)
    bullets(doc, ["Database name: plasmit_hms", "First table: tenants", "Recommended migration tool: Flyway", "Use timestamp migration naming for multi-developer work."])
    table(doc, ["Order", "Script Name", "Main Tables"], SCRIPT_ORDER)

    heading(doc, "3. High-Level ER Diagram", 1)
    doc.add_picture(str(er_image), width=Inches(7.2))
    para(doc, "This visual ER diagram shows the main relationship flow: tenant to hospital, hospital to branch, hospital to patient, patient to encounter, and encounter to clinical, diagnostics, pharmacy, billing and documents.")

    heading(doc, "4. Mermaid ER Diagram", 1)
    code(doc, ERD)

    heading(doc, "5. Module-Wise Table Catalog", 1)
    table(doc, ["Module", "Tables", "Purpose"], TABLE_CATALOG)

    heading(doc, "6. Main Table Relationships", 1)
    table(doc, ["Parent Table", "Child Table", "Relationship", "Join Key", "Business Meaning"], RELATIONSHIPS)

    heading(doc, "7. First Tables in Exact Order", 1)
    table(doc, ["Order", "Table", "Why First"], [
        ["1", "tenants", "Root SaaS customer/hospital group."],
        ["2", "hospitals", "Every hospital belongs to a tenant."],
        ["3", "branches", "Operational data is branch-specific."],
        ["4", "departments", "Hospital/branch service departments."],
        ["5", "healthcare_services", "Services offered by departments."],
        ["6", "users", "Doctors, staff and admins."],
        ["7", "roles", "Role master for RBAC."],
        ["8", "permissions", "Permission master for access control."],
        ["9", "role_permissions", "Role to permission mapping."],
        ["10", "user_branch_access", "Defines which user can access which branch."],
    ])

    heading(doc, "8. Tenant Isolation Rules", 1)
    table(doc, ["Table Type", "Required Columns", "Examples"], [
        ["Global master", "id, code, name", "country_master, gender_master"],
        ["Tenant-level", "tenant_id", "tenant_settings"],
        ["Hospital-level", "tenant_id, hospital_id", "patients, departments, medicine_master"],
        ["Branch-level", "tenant_id, hospital_id, branch_id", "appointments, encounters, lab_orders, billing_invoices, pharmacy_stock"],
        ["Encounter-level", "tenant_id, hospital_id, branch_id, patient_id, encounter_id", "vitals, diagnoses, prescriptions, lab_results"],
    ])

    heading(doc, "9. Final Recommendation", 1)
    bullets(doc, [
        "Create database plasmit_hms first.",
        "Create tenants table first.",
        "Create foundation and access-control tables before patient and clinical tables.",
        "Keep patient hospital-level and encounter branch-level.",
        "Connect lab, radiology, prescription, pharmacy, billing and documents mostly through encounter.",
        "Create FHIR and HL7 mapping tables after core transactional tables.",
...
def build_markdown(er_image: Path) -> str:
    lines = [
        ...
        "",
        f"![High-Level ER Diagram]({er_image.relative_to(DOCS).as_posix()})",
        "",
        "## Database",
        "",
    ...
    lines.extend(md_table(["Order", "Script Name", "Main Tables"], SCRIPT_ORDER))
    lines.extend(["", "## Mermaid ER Diagram", "", "```mermaid", ERD, "```", ""])
    lines.extend(["## Module-Wise Table Catalog", ""])
    lines.extend(md_table(["Module", "Tables", "Purpose"], TABLE_CATALOG))
    lines.extend(["", "## Main Table Relationships", ""])
    lines.extend(md_table(["Parent Table", "Child Table", "Relationship", "Join Key", "Business Meaning"], RELATIONSHIPS))
    lines.extend([
        "",
...
def main() -> None:
    ...
    er_image = create_er_image()
    doc = build_doc(er_image)
    doc.save(DOCX)
    MD.write_text(build_markdown(er_image), encoding="utf-8")
