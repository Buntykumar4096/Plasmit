...
export function UnitWardEscalations() {
  ...
  const [statusTimeOverrides, setStatusTimeOverrides] = React.useState<Record<string, string>>({});
  ...
  React.useEffect(() => {
    setStatusTimeOverrides((current) => {
      let changed = false;
      const next = { ...current };
      baseRows.forEach((row) => {
        if (!next[row.id]) {
          next[row.id] = unitWardEscalationStatusTime(row.sourceTime);
          changed = true;
        }
      });
  ...
  const queueRows = React.useMemo(() => baseRows
    .map((row) => ({ ...row, status: statusOverrides[row.id] ?? row.status, statusSince: statusTimeOverrides[row.id] ?? row.statusSince })), [baseRows, statusOverrides, statusTimeOverrides]);
  const escalationCounts = React.useMemo(() => ({
    all: queueRows.length,
    open: queueRows.filter((row) => row.status === "Open").length,
  ...
  const rows = React.useMemo(() => queueRows
    .filter((row) => {
      ...
      const statusMatch = statusFilter === "All"
        || (statusFilter === "Resolved" && (row.status === "Resolved" || row.status === "Closed"))
  ...
  function updateQueueStatus(row: UnitWardEscalationQueueRow, status: string) {
    ...
    setStatusTimeOverrides((current) => ({ ...current, [row.id]: unitWardEscalationClockTime(new Date()) }));
    toast.success(`${row.bedNo}: ${status}`);
  }

  function updateReviewedQueueStatus(status: string) {
    if (!activeQueueRowId) return;
    setStatusOverrides((current) => ({ ...current, [activeQueueRowId]: status }));
    setStatusTimeOverrides((current) => ({ ...current, [activeQueueRowId]: unitWardEscalationClockTime(new Date()) }));
  }
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 rounded-md border border-slate-200 bg-white p-3 shadow-sm sm:p-4 lg:grid-cols-[minmax(220px,1fr)_190px_180px_auto] lg:items-end">
        ...
        <label className="flex min-w-[150px] items-center gap-2 text-xs text-muted-foreground">
          ...
          <select className="h-9 w-full rounded-md border border-input bg-background px-3 pr-10 text-sm text-foreground outline-none focus:ring-2 focus:ring-ring/20" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
            <option value="All">All ({escalationCounts.all})</option>
            <option value="Open">Open ({escalationCounts.open})</option>
            <option value="Pending">Pending ({escalationCounts.pending})</option>
            <option value="Resolved">Resolved ({escalationCounts.resolved})</option>
          </select>
        </label>
        <Button className="h-10 self-end" variant="outline" onClick={() => { setQuery(""); setSeverityFilter("All severity"); setStatusFilter("All"); }}>Reset</Button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1180px] table-fixed border-collapse text-sm">
            ...
            <thead className="border-b border-border text-[11px] uppercase text-muted-foreground">
              <tr><th className="px-3 py-3 text-left">Patient</th><th className="px-3 py-3 text-left">Raised By</th><th className="px-3 py-3 text-left">Escalation</th><th className="px-3 py-3 text-center">Severity</th><th className="px-3 py-3 text-left">Escalation Status</th><th className="px-3 py-3 text-right">Actions</th></tr>
            </thead>
            <tbody className="divide-y divide-slate-200">
              {pagination.pageRows.map((row) => (
                <tr className="hover:bg-slate-50" key={row.id}>
                  ...
                  <td className="px-3 py-3 align-middle"><p className="font-semibold text-slate-900">{row.status}{row.statusSince ? ` (${row.statusSince})` : ""}</p><p className="mt-1 text-xs text-slate-500">Forward to: {row.forwardTo}</p></td>
                  <td className="px-3 py-3 align-middle">
      ...
      <ClinicalAlertActionDialog
        action={activeAction}
        onOpenChange={(open) => { if (!open) { setActiveAction(null); setActiveQueueRowId(null); } }}
        onComplete={(row, actionLabel) => {
          if (actionLabel === "Resolve / Close") {
            setResolvedRows((current) => new Set([...current, row.id]));
            updateReviewedQueueStatus("Closed");
          }
          if (actionLabel === "Acknowledge") {
            setAcknowledgedRows((current) => new Set([...current, row.id]));
            updateReviewedQueueStatus("Acknowledged");
          }
          if (/escalate|call/i.test(actionLabel)) updateReviewedQueueStatus("Escalated");
          toast.success(`${row.bedNo}: ${actionLabel.toLowerCase()} saved`);
          setActiveAction(null);
          setActiveQueueRowId(null);
        }}
      />
    </div>
  );
...
function unitWardEscalationFromClinicalAlert(row: ClinicalAlertRow, handledBy: string): UnitWardEscalationQueueRow {
  ...
  return { id: `alert-${row.id}`, patientId: row.alert.patientId, patientName: row.patientName, bedNo: row.bedNo, unit: row.unit, raisedBy, source: row.source, issue: row.trigger, detail: row.scenario, severity: row.severity, status: row.status, sourceTime: row.createdAt, statusSince: "", handledBy, forwardTo: clinicalAlertDefaultForwardTo(row), tone: row.tone, actionRow: row };
}
...
function unitWardEscalationFromSupervisionItem(item: SupervisionItem, handledBy: string): UnitWardEscalationQueueRow {
  ...
  return { id: `work-${item.id}`, patientId: item.patientId, patientName: item.patientName, bedNo: item.bedNo, unit: item.unit, raisedBy: item.nurse, source: item.source, issue: item.title, detail: item.detail, severity, status: item.status, sourceTime: item.due, statusSince: "", handledBy, forwardTo: clinicalAlertDefaultForwardTo(actionRow), tone, actionRow };
}



function unitWardEscalationStatusTime(sourceTime: string) {
  const now = new Date();
  const minutesAgo = sourceTime.match(/(\d+)\s*min(?:ute)?s?\s*ago/i);
  if (minutesAgo) return unitWardEscalationClockTime(new Date(now.getTime() - Number(minutesAgo[1]) * 60_000));
  const hoursAgo = sourceTime.match(/(\d+)\s*h(?:ou)?rs?\s*ago/i);
  if (hoursAgo) return unitWardEscalationClockTime(new Date(now.getTime() - Number(hoursAgo[1]) * 3_600_000));

  const clock = sourceTime.match(/\b(\d{1,2}):(\d{2})\s*(AM|PM)?\b/i);
  if (clock) {
    ...
    return unitWardEscalationClockTime(time);
  }

  return unitWardEscalationClockTime(now);
}



function unitWardEscalationClockTime(value: Date) {
