...
// useSyncExternalStore requires the same snapshot reference until the store
// actually changes. Reading and parsing localStorage on every render creates a
// new object each time, which React correctly treats as an endless update.
const serverSnapshot = initialState();
let cachedState: AdmissionStoreState | undefined;
...
function readState(): AdmissionStoreState {
  ...
  try {
    ...
    cachedState = { ...initialState(), ...JSON.parse(saved) } as AdmissionStoreState;
  } catch {
    cachedState = initialState();
  }
...
export function useAdmissionStore() {
  const state = React.useSyncExternalStore(subscribeToAdmissionStore, readState, () => serverSnapshot);
